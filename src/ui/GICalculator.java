...
public class GICalculator {
     ...
     * @param: multreac double Must be 1.5 or 2 (double-type number)
     ...
     * @param: elm double Must be a double-type number that has been previously initialized
     * @param: multadd double Must be 1.15 or 1.25 (double-type number)
     ...
     * @param: lvl double Must be a double-type number that has been previously initialized
     * @param: elm double Must be a double-type number that has been previously initialized
     * @param: multtra double Must be 0.25, 0.5, 0.6, 1.2, 1.5, 2 or 3 (double-type number)
     * @param: multres double Must be a double-type number that has been randomly selected
    ...
    private static double traCalculus() {
        System.out.println("Please enter character's level & elemental mastery");
        lvl = txt.nextDouble();
        elm = txt.nextDouble();
            ...
            if (menuOpt == 1) { // We use conditionals to filter options
                ...
                if (quantOpt.equals("ch")) {
                    ...
                    for (int i = 0; i < tempCom.size(); i++) {
                        ...
                        comSum+=tempCom.get(i);
                    }
                    System.out.println("The sumatory of total damage is: " + comSum);
                    tempCom.clear();
                    comSum=0;
                }
            } else if (menuOpt == 2) {
                dmgRating(); // Display the list of the latest 10 (or fewer) calculated damages
            } else if (menuOpt == 3) {
                dmgHigh(); // Display the highest damage calculated so far
            } else if(menuOpt == 0){
